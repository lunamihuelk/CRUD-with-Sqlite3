...
def createDataBase(name_base="base.db"):
	conn = sql.connect(name_base)
	conn.commit()
	conn.close()
...
def insertRow(name_tabla, session, clase, puntaje):
	conn = sql.connect("stream.db")
	cursor = conn.cursor()
	instruccion = f"INSERT INTO {name_tabla} VALUES ('{session}', '{clase}', '{puntaje}')"
	cursor.execute(instruccion)
	conn.commit()
	conn.close()
...
def insertFilas(name_tabla, streamerslist):
	conn = sql.connect("streamers.db")
	cursor = conn.cursor()
	instruccion = f"INSERT INTO '{name_tabla}' VALUES (?, ?, ?)"
